...
public class PushInstructionBuilderTest {
    ...
    public void setUp() throws FlowParserException {
        ...
        when(actionBuilder.makeOutAction(eq("Port"), eq(VLanTagAction.PUSH), any(VlanId.class)))
                .thenReturn(actionList);
        instructionList =
                instructionBuilder.makeInstructions("Port", VLanTagAction.PUSH, mock(VlanId.class));
    }

    @Test
    public void testCall() throws FlowParserException {
        verify(actionBuilder).makeOutAction(eq("Port"), eq(VLanTagAction.PUSH), any(VlanId.class));
    }
}
